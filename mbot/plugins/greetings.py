...
def load_settings():
    if os.path.exists(SETTINGS_FILE):
        with open(SETTINGS_FILE, "r") as f:
            return json.load(f)
    return {"default": {"audio_output": "mp3", "quality": "high"}, "users": {}}
...
# Command to handle settings
@Mbot.on_message(filters.command("settings"))
@maintenance_check
async def settings_command(client, message):
    user_id = str(message.from_user.id)
    user_settings = settings["users"].get(user_id, settings["default"])

    keyboard = [
        [
            InlineKeyboardButton(f"Audio Output: {user_settings['audio_output']}", callback_data="change_audio_output"),
            InlineKeyboardButton(f"Quality: {user_settings['quality']}", callback_data="change_quality")
        ],
        [InlineKeyboardButton("❌ Close", callback_data="close")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await message.reply_text("⚙️ Settings:", reply_markup=reply_markup)

# Handle callback queries for settings
@Mbot.on_callback_query(filters.regex(r"change_audio_output"))
async def change_audio_output(client, callback_query):
    user_id = str(callback_query.from_user.id)
    user_settings = settings["users"].get(user_id, settings["default"])

    new_output = "mp3" if user_settings["audio_output"] == "flac" else "flac" if user_settings["audio_output"] == "m4a" else "m4a"
    user_settings["audio_output"] = new_output
    settings["users"][user_id] = user_settings
    save_settings(settings)

    await callback_query.answer(f"Audio output changed to {new_output}", show_alert=True)
    await settings_command(client, callback_query.message)

@Mbot.on_callback_query(filters.regex(r"change_quality"))
async def change_quality(client, callback_query):
    user_id = str(callback_query.from_user.id)
    user_settings = settings["users"].get(user_id, settings["default"])

    new_quality = "high" if user_settings["quality"] == "medium" else "medium" if user_settings["quality"] == "low" else "low"
    user_settings["quality"] = new_quality
    settings["users"][user_id] = user_settings
    save_settings(settings)

    await callback_query.answer(f"Quality changed to {new_quality}", show_alert=True)
    await settings_command(client, callback_query.message)
