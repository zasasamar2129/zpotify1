...
async def song(client, message):
    ...
    if is_maintenance_mode() and message.from_user.id not in SUDO_USERS:
        await message.reply_text(SAAVN_RESPONSES.get(user_lang, {}).get("maintenance","🔧 The bot is under maintenance. Please try again later."))
        return
    
    # Check Banned Users
    if message.from_user.id in banned_users:
        await message.reply_text(SAAVN_RESPONSES.get(user_lang, {}).get("banned","You are banned from using this bot  ദ്ദി ༎ຶ‿༎ຶ ) "))
        return

    try:
       args = message.text.split(None, 1)[1]
    except:
        return await message.reply(SAAVN_RESPONSES.get(user_lang, {}).get("Saavn_Argument","/saavn requires an argument."))
    if args.startswith(" "):
        await message.reply(SAAVN_RESPONSES.get(user_lang, {}).get("Saavn_Argument","/saavn requires an argument."))
        return ""
    pak = await message.reply(SAAVN_RESPONSES.get(user_lang, {}).get("Downloading", 'Downloading...'))
    try:
  ...
  #  album_id = r.json()[0]["albumid"]
    ...
    await pak.edit(SAAVN_RESPONSES.get(user_lang, {}).get("Uploading", 'Uploading...'))
